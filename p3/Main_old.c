...
-----------------------------  SleepingBarber  ---------------------------------
 ...
 var 
   ...
   thread: array[22] of Thread = new array of Thread {22 of new Thread}
 ...
 function SleepingBarberTest()
    ...
    thread[0].Fork(barber, 0)
    thread[1].Fork(barber, 1)
    
    -- Customer threads initialization
    for i = 2 to 21
      thread[i].Fork(customer, i)
    endFor 
    
 endFunction
 
  -- Barber thread function
  function barber(barberID: int)
  ...
  function customer(customerID: int)
    ...
    if waiting < CHAIRS			-- Customer leaves if there are no empty chairs
       ...
       get_haircut(customerID)		-- Be seated in the barber chair and get haircut
    else
  ...
  function get_haircut(customerID: int)
  ...
  function cut_hair (barberID: int)
