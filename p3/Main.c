...
var 
  ...
  -- Function to call the barber method
  function barberCall (barberID: int)
    barbershop.barber(barberID)
  endFunction

  -- Function to call Customer method 10 times each
...
var
  ...
  behavior GamingParlor
    ...
    method Init ()
      ...
      -- This is the maximum number of dice available in the gaming parlor frontdesk
      availDice = 8
      -- Indicates the number of threads waiting for the required number of dice to become available
      waitingGroups = 0
    ...
    -- The Return method signals the nextgroup condition variable which ensures
    -- that the threads are served on First come First served basis
    method Request (numberofDice: int)
      ...
      -- check if there are more than one group and adds itself at the back of the "waitList"
      if waitingGroups > 1
        waitList.Wait (&mLock) 
      endIf

      -- checks if there is enough dice to play the game and waits in the "nextGroup"
      while availDice < numberofDice
      ...
      -- Signals the waitList condition variable to add the leading thread to the nextGroup queue
      waitList.Signal (&mLock)
    ...
    -- This method returns the dice and signals the nextGroup condition variable
    -- to check if there are enough dice for it to start playing
