...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    -- semaphore variable to show if buffer is full
    fullBuf: Semaphore = new Semaphore
    -- semaphore variable to show if buffer is empty
    emptyBuf: Semaphore = new Semaphore
    -- Mutex lock for Producer variables
    lockP: Mutex = new Mutex 
    -- Mutex lock for Consumer variables
    lockC: Mutex = new Mutex 
    -- Mutex lock for Buffer
    lockB: Mutex = new Mutex 
  ...
  function ProducerConsumer ()
      ...
      lockC.Init()
      lockB.Init()
      -- Initialize emptyBuf semaphore variable to 0 indicating Buffer is completely empty
      emptyBuf.Init(0)
      -- Initialize fullBuf semaphore variable to BUFFER_SIZE to indicating there are BUFFER_SIZE number of free spots in the buffer
      fullBuf.Init(BUFFER_SIZE)
  ...
  function Producer (myId: int)
	...
	-- Check to see if the buffer is full. If it is then the fullBuf semaphore will ensure that the current Producer thread goes to sleep
        fullBuf.Down()
        
	-- Acquire lock to the Buffer variable since it is shared between multiple Producers and Consumers
        -- Add c to the buffer
	lockB.Lock()
        buffer [bufferNextIn] = c

        -- Acquire lock to the Producer thread shared variables
        lockP.Lock()
        
	bufferNextIn = (bufferNextIn + 1) % BUFFER_SIZE
        bufferSize = bufferSize + 1
        
	lockP.Unlock() -- Release lock to the Producer thread shared variables

        -- Print a line showing the state
	PrintBuffer (c)
	
	-- Release lock to the Buffer variable
	lockB.Unlock()
        
        -- Perform synchronization...
        -- wakes up the Consumer Thread
	emptyBuf.Up()
  ...
  function Consumer (myId: int)
      ...
      while true
        -- Perform synchroniztion...
	-- Check to see if the buffer is empty. If it is, then the emptyBuf semaphore will ensure that the current Consumer thread goes to sleep
        emptyBuf.Down()
	...
	-- lockB.Unlock()
        -- Acquire lock to the Consumer thread shared variables
	lockC.Lock()	
        
	bufferNextOut = (bufferNextOut + 1) % BUFFER_SIZE
        bufferSize = bufferSize - 1
        
	lockC.Unlock() -- Release the consumer thread lock
      
        -- Print a line showing the state
        PrintBuffer (c)
	lockB.Unlock() -- Release lock to the Buffer variable
        
        -- Perform synchronization...
	fullBuf.Up() -- wakes up the Producer thtread when there is an empty slot
      endWhile
    endFunction
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  class ForkMonitor
    ...
    fields
      ...
      beginEating: array [5] of Condition  -- Condition variable for philosophers to begin eating
      monLock: Mutex                       -- Lock for monitor
    methods
      Init ()
      PickupForks (p: int)
      PutDownForks (p: int)
  ...
  behavior ForkMonitor
    ...
    method Init ()
      -- Initialize so that all philosophers are THINKING.
        ...
        status = new array [5] of int {5 of THINKING}
        -- Initialize Condition variables
        beginEating = new array [5] of Condition {5 of new Condition}
        for i=0 to 4
          beginEating[i].Init()
        endFor
    ...
    method PickupForks (p: int)
      -- This method is called when philosopher 'p' is wants to eat.
        -- Acquire lock of the monitor
        monLock.Lock()
        -- Set Philosopher's state to EATING
        status [p] = HUNGRY
        -- Print current status of the Philosophers
        self.PrintAllStatus()
        -- Check if the Forks are available to use
        self.eatCheck(p)
        -- If forks are not available then we put the current thread to sleep
        if status[p] != EATING
          beginEating [p].Wait (& monLock)
        endIf
        -- Release the monitor's lock
        monLock.Unlock()
      endMethod

    method PutDownForks (p: int)
      -- This method is called when the philosopher 'p' is done eating.
        -- Acquire the lock of the monitor
        monLock.Lock()
        -- Set the current Philosopher's state to THINKING
        status[p] = THINKING
        -- Print current status of the Philosophers
        self.PrintAllStatus()
        -- Check if the right neighbor has both forks and can we woken up
        self.eatCheck((p+1) % 5)
        -- Check if the left neighbor has both forks and can we woken up
        self.eatCheck((p+4) % 5)
        -- Release the monitor's lock
        monLock.Unlock()
      endMethod

    method eatCheck (p: int)
      -- We check if the current philosopher is HUNGRY and the neighbors are eating. If either neighbors are eating it means that the current philosopher cannot start eating
      if status [p] == HUNGRY && status [(p+1) % 5] != EATING && status [(p-1) % 5] != EATING
        status [p] = EATING
        self.PrintAllStatus()
	beginEating[p].Signal (&monLock)
      endIf
      endMethod
