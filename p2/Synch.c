...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
	  ...
	  state = UNLOCKED
	  -- List of Threads currently vying for the mutex
	  waitingThreads = new List [Thread]
	endMethod
      ...
      method Lock ()
          -- variable declaration for previous Interrupt status
          var prevInterruptStat: int

	  -- returns previous Interrupt status and disables the Interrupt
	  prevInterruptStat = SetInterruptsTo (DISABLED)
	
	  -- checks whether the mutex is locked. 
	  -- If mutex is locked, add current thread to the waiting Threads List and put the current thread to sleep
	  while state == LOCKED
	    waitingThreads.AddToEnd (currentThread)
	    currentThread.Sleep()
	  endWhile
	  
	  -- additional check for mutex being locked
	  if heldBy != null
	    FatalError ("Lock already held")
	  endIf
	  
	  -- set mutex state to LOCKED and sets heldBy field to the current thread
	  state = LOCKED
	  heldBy = currentThread
	
	  -- restore Interrupt status to previous state
	  prevInterruptStat = SetInterruptsTo (prevInterruptStat)
      endMethod

      ----------  Mutex . Unlock  ----------

      method Unlock ()
          -- variable declaration for previous Interrupt status and nextThread pointer
          var
	    prevInterruptStat: int
	    nextThread: ptr to Thread	  
	  
	  -- returns previous Interrupt status and disables the Interrupt
	  prevInterruptStat = SetInterruptsTo (DISABLED)
	  
	  -- checks if the mutex is already UNLOCKED
	  if state == UNLOCKED
	    FatalError ("Nothing to unlock")
	  endIf
	
	  -- check to see if the current thread holds the lock
	  if heldBy != currentThread
	    FatalError ("Lock not in currentThread")
	  endIf

	  -- releases the lock and sets heldBy to null
	  state = UNLOCKED
	  heldBy = null
	  
	  -- removes earliest waiting thread from the list and adds it to the ready list
      ...
      method IsHeldByCurrentThread () returns bool
          -- Is current thread holding the lock? If yes, returns true else false
