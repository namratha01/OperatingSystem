...
*/
      ...
      -- Initialize the HoareThreadManager
      hoarethreadManager = new HoareThreadManager
...
-----------------------------  RunHoareThreadManagerTests  ---------------------------------
--
-- This function tests the HoareThreadManager.  It creates a bunch of threads
-- and starts each thread running. Each thread will execute
-- the "TestHoareThreadManager" function.  The main thread will then wait until all
...
--
  function RunHoareThreadManagerTests ()
      ...
      -- Prints the current status of the freeList of the the HoareThreadManager threads
      hoarethreadManager.Print ()
      
      th = alloc Thread
      th.Init ("THTM1")
      th.Fork (TestHoareThreadManager, 1)
      th = alloc Thread
      th.Init ("THTM2")
      th.Fork (TestHoareThreadManager, 2)
      th = alloc Thread
      th.Init ("THTM3")
      th.Fork (TestHoareThreadManager, 3)
      th = alloc Thread
      th.Init ("THTM4")
      th.Fork (TestHoareThreadManager, 4)
      th = alloc Thread
      th.Init ("THTM5")
      th.Fork (TestHoareThreadManager, 5)
...
-----------------------------  TestHoareThreadManager  ---------------------------------
--
-- This function is the main function for a thread which will test the
-- ThreadManager using Hoare Condition.  It will request and return Thread objects.  First, it
-- grabs a unique number and stuffs it in the Thread.
--
  function TestHoareThreadManager (myID: int)
      var i, j: int
          th: ptr to Thread
      printIntVar ("\nThread started", myID)
      for i = 1 to 1
        
        th = hoarethreadManager.GetANewThread ()
        for j = 1 to 2 * 2 * WAIT_TIME+i
          currentThread.Yield ()
        endFor
        
        hoarethreadManager.FreeThread (th)
