code Kernel
  ...
  -- Namratha Basavanahalli Manjunatha

------------------------------  InitFirstProcess  ------------------------------

  function InitFirstProcess ()
    var
      -- Create a ptr to Thread variable for the user thread
      userprogramThread: ptr to Thread
    
    -- Create a new thread object by invoking GetANewThread method
    userprogramThread = threadManager.GetANewThread()
    -- Initialize the newly created thread with name "UserProgram"
    userprogramThread.Init ("UserProgram")
    -- Start the new thread with the StartUserProcess function by invoking Fork
    userprogramThread.Fork (StartUserProcess, 0)

  endFunction

------------------------------  StartUserProcess  ------------------------------

  function StartUserProcess (sup: int)

    var
      -- Create a ptr to ProcessControlBlock variable for the new process
      p: ptr to ProcessControlBlock
      initPC: int
      InitUserStackTop: int
      -- Create a ptr to int variable to hold the pointer to the full-sized system stack
      initSystemStackTop: ptr to int
      filePtr: ptr to OpenFile      
 
    -- Create a new process within the user thread
    p = processManager.GetANewProcess ()

    -- The following two commands link the user process and user thread to each other
    p.myThread = currentThread
    currentThread.myProcess = p

    -- The following set of commands opens the TestProgram1 file, and reads the program into the 
    -- logical address space of the user process
    filePtr = fileManager.Open ("TestProgram1")
    print ("Loading initial program...\n")
    initPC = filePtr.LoadExecutable (& p.addrSpace)
    fileManager.Close (filePtr)

    -- InitUserStackTop holds the top of the user stack which begins after the end of
    -- the logical address space    
    InitUserStackTop = p.addrSpace.numberOfPages * PAGE_SIZE
    initSystemStackTop = & currentThread.systemStack[SYSTEM_STACK_SIZE-1]    

    -- The following set of commands will now turn the current thread into a user-level thread
    -- by changing to the user page table and setting the currentThread.isUserThread to true
    -- And of course we disable the interrupts to avoid any race conditions between competing threads
    currentInterruptStatus = DISABLED
    p.addrSpace.SetToThisPageTable ()
    currentThread.isUserThread = true

    -- The following method will change the mode bits and 
    -- perform the jump to user program execution "atomically"
...
-----------------------------  HoareMutex  ---------------------------------
  ...
  behavior HoareMutex
    ...
    --         free and then get it.
    --    Unlock()
    --         Release the mutex and hands over the lock to the current thread
    --    Init()
    --         Each mutex must be initialized.
    --    IsHeldByCurrentThread()
    --         Return TRUE iff the current (invoking) thread holds a lock
...
-----------------------------  Hoare Condition  ---------------------------------
  ...
  behavior HoareCondition
    ...
    -- means that in the time between a Signal() operation and the awakening
    -- and execution of the corrsponding waiting thread, other threads cannot
    -- sneak in and run.
    --
    -- This class provides the following methods:
    --    Wait(mutex, semaphore, int)
    --         When the threads decide to Wait on a condition variable
    --         a priority is assigned to it which lets us know which thread needs to be executed first
    --    Signal(mutex, semaphore, int)
    --         When a thread Signals another thread, the thread with the highest priority as shown by
    --         nextCount variable is signaled for execution
    --    Init()
    --         Each condition must be initialized.
      ...
      method Init ()
	  countVar = 0
          hoareSem = new Semaphore
          hoareSem.Init(0) 
        endMethod

      ----------  HoareCondition . Wait  ----------

      method Wait (mutex: ptr to HoareMutex, threadSem: ptr to Semaphore, nextCount: int)
          var
            oldIntStat: int
          ...
          -- This block of code achieves the actual Wait logic using semaphore, hoareSem and
          -- a HoareMutex. 
          countVar = countVar + 1
          if nextCount > 0
            threadSem.Up ()
      ...
      method Signal (mutex: ptr to HoareMutex, threadSem: ptr to Semaphore, nextCount: int)
          var
            oldIntStat: int
          ...
          -- This block of code implements the Signal logic using semaphore (hoareSem) and a HoareMutex
          if countVar > 0
...
-----------------------------  HoareThreadManager  ---------------------------------
  ...
  behavior HoareThreadManager
      ...
      method Init ()
        ...
        -- the one and only "HoareThreadManager" object.
        -- 
	  -- var
          --  i: int

          print ("Initializing Hoare Thread Manager...\n")
          
          -- Initialize the semaphore that will used by the condition variable
          threadSem = new Semaphore
          threadSem.Init(0)
          ...
          -- Initialize monitor's lock 
          threadManagerLock = new HoareMutex
          threadManagerLock.Init()
          
          -- Create a list of threads. We are using 2 for our purposes here
          threadTable = new array of Thread {2 of new Thread }
          threadTable[0].Init("t0")   
          --threadTable[1].Init("t1")   
      
          -- Set status of each thread to UNUSED and add to freeList 
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- 
          ...
          -- Acquire the monitor's lock to ensure that access to class and global variables are synchronized
          threadManagerLock.Lock ()

          -- Wait on condition variable, aThreadBecamefree, until
          -- a thread is added to the freeList
          -- Note that the condition variable aThreadbecamefree is a Hoare condition variable
          while freeList.IsEmpty ()
            print ("\nGetANewThread - Wait: ")
            print (currentThread.name)
            
            aThreadBecameFree.Wait (&threadManagerLock, &threadSem, self.nextCount)
            
            print ("\n")
            -- Printing the status of th freeList and threads
            self.Print ()
          endWhile          

          -- Removes the thread from the freeList and
          -- Set thread status to JUST_CREATED and returns the thread pointer
          ...
          -- This block of code ensures that the Lock is unlocked by the thread
          -- that was offered by the signaling thread
          if self.nextCount > 0
            threadSem.Up()
          else
      ...
      method FreeThread (th: ptr to Thread)
        ...
        --
          ...
          -- change the status to UNUSED once the thread is free
          -- and ddd that thread to freeList
          th.status = UNUSED
          freeList.AddToEnd (th)

          -- Signals the condition variable aThreadBecameFree which was waiting 
          -- in GetANewThread entry method that the thread has become free
          print ("\nFreeThread - Before Signal: ")
	  print (currentThread.name)
          
          aThreadBecameFree.Signal (&threadManagerLock, &threadSem, self.nextCount)
          
          print ("\n")
          self.Print ()

          -- This block of code ensures that the mutex lock is released only in case 
          -- no other threads at waiting for it
          if self.nextCount > 0
            threadSem.Up()
          else
            threadManagerLock.Unlock()
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
	  ...
	  var
          ...
          -- Create a list of threads , MAX_NUMBER_OF_PROCESSES = 10 
          ...
          -- Initialize status to UNUSED and added all the threads in the threadTable array on the freeList
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- 
          ...
          -- Release the monitor's lock that we acquired earlier in this method
          threadManagerLock.Unlock ()
      ...
      method FreeThread (th: ptr to Thread)
        ...
        --
          ...
          -- change the status to UNUSED once the thread is free
          -- and Add that thread to freeList
          th.status = UNUSED
          freeList.AddToEnd (th)
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        --
          ...
          -- Initialize list of process control blocks to be added to when process is free
          freeList = new List [ProcessControlBlock]

          -- Create an array of process control blocks
          processTable = new array of ProcessControlBlock {MAX_NUMBER_OF_PROCESSES of new ProcessControlBlock}
          
          -- Initialize a condition variable for when the process becomes free
          aProcessBecameFree = new Condition
          aProcessBecameFree.Init()

          -- Initialize a condition variable for when the process terminates
          aProcessDied = new Condition
          aProcessDied.Init()

          -- Initialize the monitor's lock
          processManagerLock = new Mutex
          processManagerLock.Init()

          -- Initialize a variable to generate the process ID for the newly created processes
          nextPid = 0          
         
          -- Initialize each of the process control blocks in the processTable array
          processTable[0].Init()
          processTable[1].Init()
          processTable[2].Init()
          processTable[3].Init()
          ...
          -- Initialize each process control block's status to FREE and add to the freeList
          for i = 0 to MAX_NUMBER_OF_PROCESSES-1
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        -- on the condition variable, aProcessBecameFree, until one is available.
        --
          ...
          -- Wait on condition variable, aProcessBecamefree, until
          -- a process control block is added to the freeList
          while (freeList.IsEmpty())
            aProcessBecameFree.Wait (&processManagerLock)
          endWhile

          -- When FreeProcess method signals the addition of a process control block to the freeList,
          -- increment and assign the nextPid value to the process control block's process id
          -- Next remove the process control block from freeList, change it's status to ACTIVE
          -- and return the pointer to the process control block
          p = freeList.Remove ()
          p.status = ACTIVE
          p.pid = nextPid
          nextPid = nextPid + 1
          ...
          -- Return the pointer to the process control block
          return p
        endMethod
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        --
          ...
          -- Set status of the process control block to FREE 
          -- and add the process control block to the freeList
          p.status = FREE
          freeList.AddToEnd (p)
          
          -- Signal to the condition variable aProcessBecameFree which was waiting 
          -- in GetANewProcess entry method that a process control block is now available in the freeList
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
          --
          -- Allocates one frame and returns address of the frame 
          --
          var i, pageOffset, frameAddr: int
          
          -- Acquire the lock to the frameManager data structure
          frameManagerLock.Lock ()
           
          -- Wait on the condition variable, newFramesAvailable until
          -- the required number of frames are available
          while numberFreeFrames < numFramesNeeded
            newFramesAvailable.Wait (&frameManagerLock)
          endWhile
          
          -- For each frame that is returned, set the framesInUse
          for i = 0 to numFramesNeeded - 1
            pageOffset = framesInUse.FindZeroAndSet()
            -- Compute frameAddr from the PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME and PAGE_SIZE
            frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (pageOffset * PAGE_SIZE)
            -- Set the frameaddress in the page table based on the frameAddr computer above
            aPageTable.SetFrameAddr (i, frameAddr)
          endFor
         
          -- Update the numberFreeFrames to reflect the actual remaining number of free frames
          -- after the frames to be returned are determined
          numberFreeFrames = numberFreeFrames - numFramesNeeded
         
          -- Update the numberOfPages in the Pagetable to show the number of frames being returned
          aPageTable.numberOfPages = numFramesNeeded

          -- Release the lock to the frameManager data structure
          frameManagerLock.Unlock ()
        endMethod
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
          -- 
          -- Returns the frames for other processes. 
          -- Once frames are returned, we do a Broadcast to signal to all the processes waiting against
          -- the newFramesAvailable condition variable
          --
          ...
          -- For each page frame clear the framesInUse field and free them from the processes
          ...
          -- Update the numberFreeFrames to show the freed page frames
          numberFreeFrames = numberFreeFrames + aPageTable.numberOfPages
          
          -- Broadcast to all the processes that are waiting on the frames that they are ready for consumption
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      ...
      -- From the given virtual address space (filename), we extract the characters into an array variable (strBuffer)
      -- MAX_STRING_SIZE limits the number of characters
      i = currentThread.myProcess.addrSpace.GetStringFromVirtual ( & strBuffer, filename asInteger, MAX_STRING_SIZE)

      -- We check to see if the characters extracted are valid. If invalid, then return -1 to indicate failure
      if i < 0
        return -1
      endIf

      -- Initialize the new address space for the Exec handler
      newAddrSpace.Init ()
      
      -- The following set of instructions open and load the extracted "filename" into
      -- the logical address space of the process
      -- There is also a check to ensure that there exists a file that can be loaded into the logical address space
      ...
      -- Return all the frames of the currently running process so that we can overwrite it with the loaded process space
      frameManager.ReturnAllFrames(&currentThread.myProcess.addrSpace)
      currentThread.myProcess.addrSpace = newAddrSpace
 
      -- The following commands initialize the top of the User stack and the System Stack
